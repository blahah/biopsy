...
// main plot
var margin = {top: 10, right: 20, bottom: 20, left: 54},
...
// top plot
var tmargin = {top: 10, right: 20, bottom: 5, left: 54},
...
// left plot
var lmargin = {top: 10, right: 5, bottom: 20, left: 15},
...
function loadDataForObjective(objective) {
  ...
  d3.csv("data/" + objective + ".csv", function(error, data) {
    ...
    iterid_max = d3.max(data, function(d) { return round(d.iterid); });
    score_max = d3.max(data, function(d) { return round(d.score); });

    objectivedata[objective] = {
      'runs': runs,
      'topdata': topdata,
      'iterid_max': iterid_max,
      'score_max': score_max
    };
...
function updatePlotsForObjective(objective) {
  ...
  x.domain([0, data['iterid_max']]);
  y.domain([0, data['score_max']]);
...
function updateMain(runs) {
  ...
  // data join
  run = mainsvg.selectAll(".run-line")
      .data(runs);
...
function updateLeft() {
  ...
  lx.domain([0, d3.max(lhistdata, function(d) { return round(d.y); })]);
  ...
  // update
  var barheight = height - y(lhistdata[0].dx);
...
function updateTop(topdata) {
  ...
  ty.domain([0, d3.max(tophistdata, function(d) { return round(d.y); })]);
  ...
  // update
  bars.attr("width", x(tophistdata[0].dx) - 2)
      .attr("height", 0)
      .transition().delay(500)
      .attr("transform", function(d) {
        return "translate(" +
