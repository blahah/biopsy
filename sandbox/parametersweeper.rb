...
class ParameterSweeper
  def initialize(options, constructor=false)
  ...
  def run(groupsize, continue_on_crash=false)
    Dir.chdir('outputdata_refactor1') do
      ...
      @input_combinations.each do |parr|
        ...
        # add an additional key :assembly_number passing the assembly number that is being passed to the constructor
        # overall structure:
        #{
        # :parameters => 'chaning parameters stored as hashes' <- ranges of parameters dictated in options argument
        # :settings => 'unchanging settings stores as hashes' <- dictated in the options argument
        # :assembly_number => 'value of assembly number currently being passed to constructor'
        #}
        cmd = @constructor.call({:parameters => parr[0]}.merge!({:settings => @input_parameters[:settings]}).merge!({:assembly_number => parr[1]}))
        ...
        # output progress
        if parr[1]%1000==0
          puts "Currently on #{parr[1]} / #{output_parameters.length}. This run took #{time}"
        end
        # assembly decides the directory group in which output file will be placed
        groupceil = (parr[1] / groupsize).ceil * groupsize
        ...
        Dir.mkdir("#{destdir}/#{parr[1]}") unless File.directory?("#{destdir}/#{parr[1]}")
        # loop through output files from soap and move output files to relevent directory
        Dir["#{parr[1]}.*"].each do |file|
          # Dir['#{.parr[1]}.*'] will grab the directory group file (destdir) of the first output in each destdir file and attempt to gzip
          if file == destdir then
            next
          end
          `gzip #{parr[1]}.* 2> /dev/null`
          file = file.gsub(/\.gz/, '')
          # move produced files to directory group
          FileUtils.mv("#{file}.gz", "#{destdir}/#{parr[1]}")
        ...
        CSV.open("filenameToParameters.csv", "ab") do |csv|
          mutex.synchronize do
           ...
           csv << [parr[1]] + parr[0].map{|key, value| value} + [time]
          end
        end
      end
    end
...
soap_constructor = Proc.new { |input_hash|  # make config file if doesn't already exist
  ...
  constructor = "#{input_hash[:settings][:SOAP_file_path]} all -s soapdt.config -o #{input_hash[:assembly_number]}"
