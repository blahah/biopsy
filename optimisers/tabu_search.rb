...
module BiOpSy
  ...
  class Distribution
    ...
    def initialize(mean, range, sd_increment_proportion, starting_sd_divisor)
      @mean = mean
      @sd = range.size / starting_sd_divisor # this is arbitrary - should we learn a good initial setting?
      @range = range
  ...
  class TabuSearch #< OptmisationAlgorithm
    ...
    def initialize(parameter_ranges, threads=8, limit=nil)
      ...
      @backtrack_cutoff = @max_hood_size * 3
    ...
    def backtrack_or_continue
      if @iterations_since_best * @max_hood_size >= @backtrack_cutoff
        @iterations_since_best = 0
